...
const Dashboard = ({ movie }) => {
  ...
  const rating_percent =
    "vote_average" in movie ? Math.ceil(movie.vote_average) * 10 : null;

  const movie_lang =
    movie.spoken_languages &&
    movie.spoken_languages.map(
      (lang) => get_lang_name(lang.iso_639_1) || lang.name
    );
  ...
  const backdrop_style = {
    backgroundImage: movie.backdrop_path
      ? `url("${get_backdrop_url(
          movie.backdrop_path,
          2
        )}"), linear-gradient(transparent, rgb(30,30,40) 50%, black)`
    ...
    <div style={backdrop_style} className="backdrop movie-page_dashboard">
      <div className="movie-page_dashboard-poster-section">
        <div className="poster-bg">
          {movie.poster_path && (
            <img
              src={get_poster_url(movie.poster_path, 2)}
              alt="movie poster"
            />
          )}
        </div>
      ...
      <div className="movie-page_dashboard-info">
        ...
        {movie.genres && (
          <div className="movie-page_genres">
...
const Profile = ({ cast }) => {
  const get_profile_pic = () => {
    if (cast.profile_path) return get_profile_url(cast.profile_path);
    else if (cast.gender) {
      ...
      <div className="modal-dialog modal-lg modal-dialog-scrollable">
        <div className="modal-content">
          ...
          <div className="modal-body">
              ...
              {cast_list &&
                cast_list.map((cast, i) => (
                  <li key={i}>
                    ...
                    <span className="cast-role">{cast.character}</span>
                  </li>
                ))}
            </ul>
            ...
            <ul className="cast-list">
              {crew_list &&
                crew_list.map((crew, i) => (
                  <li key={i}>
                    <Profile cast={crew} />
                    <span className="cast-name">{crew.name}</span>
                    <span className="cast-role">{crew.job}</span>
                  </li>
                ))}
            </ul>
          </div>
    ...
    <React.Fragment>
      {cast_list && (
        <div className="section">
          ...
          <button
            className="more-cast-btn"
            data-toggle="modal"
            data-target="#fullCastCrewModal"
          >
            See more...
          </button>
...
const MoviePage = ({ match }) => {
  ...
  useEffect(() => {
    movieData.apiCall(
      `/${media_type}/${id}?append_to_response=credits,external_ids,videos,recommendations,similar,reviews&language=en`
    );
      ...
      {movieData.error && (
        <div className="text-center">
          <div className="error-msg">
            {movieData.errorMsg ? movieData.errorMsg : "Something went wrong!"}
          </div>
